...
public class EnderButtListener implements Listener {
    ...
    public void onPlayerInteract(PlayerInteractEvent event) {
        ...
        if (FlowerHub.getInstance().getConfig("listeners.yml").getBoolean("enderbutt.enabled", true)) {
            if (action == Action.RIGHT_CLICK_AIR || action == Action.RIGHT_CLICK_BLOCK) {
                Player player = event.getPlayer();
                if (event.getItem() != null && event.getItem().getType() == Material.ENDER_PEARL) {
                    if (FlowerHub.getInstance().getConfig("listeners.yml").getString("enderbutt.type").equals("FOLLOW")) {
                        event.setUseItemInHand(Event.Result.DENY);
                        event.setUseInteractedBlock(Event.Result.DENY);

                        player.setVelocity(player.getLocation().getDirection().normalize().multiply(2.5F));

                        if (FlowerHub.getInstance().getConfig("listeners.yml").getBoolean("enderbutt.sound_enabled")) {
                            String sound = FlowerHub.getInstance().getConfig("listeners.yml").getString("enderbutt.sound");
                    ...
                    } else if (FlowerHub.getInstance().getConfig("listeners.yml").getString("enderbutt.type").equals("RIDE")) {
                        ...
                        if (FlowerHub.getInstance().getConfig("listeners.yml").getBoolean("enderbutt.effect-ride.enable")) {
                            String effectValue = FlowerHub.getInstance().getConfig("listeners.yml").getString("enderbutt.effect-ride.effect");
                            ...
                            }).runTaskTimer(FlowerHub.getInstance(), 1L, 1L);
                        }
                    }
                }
            }
