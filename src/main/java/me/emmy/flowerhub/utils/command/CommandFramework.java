...
public class CommandFramework implements CommandExecutor {
    ...
    private FlowerHub plugin;

    public CommandFramework(FlowerHub plugin) {
        this.plugin = plugin;
        ...
        for (int i = args.length; i >= 0; i--) {
            ...
            if (commandMap.containsKey(cmdLabel)) {
                ...
                if (!command.permission().equals("") && (!sender.hasPermission(command.permission()))) {
                    sender.sendMessage(CC.translate(FlowerHub.getInstance().getConfig("messages.yml").getString("messages.no-permission"))); // TODO change
                    return true;
                }
    ...
    private void defaultCommand(CommandArgs args) { // Emmy says Hi :)
        ...
        if (parts.length > 1) {
            ...
            args.getSender().sendMessage(CC.translate(FlowerHub.getInstance().getConfig("messages.yml").getString("anti-syntax").replace("%argument%", args.getLabel())));
        }
    }
}
